...
| （自由文） | AI秘書チャッピーが自然返信（会社knowledgeを参照） |

会話状態（司令塔の数字選択）は約24時間で期限切れです。
自由会話履歴は直近約16件を保持します。

### 会社知識（knowledge）
- リポジトリ直下の `knowledge/*.md` を LINE 自然会話の直前に読み込み、システムプロンプトへ追加します
- 一般知識より knowledge を優先。未登録は「まだ登録されていません」
- Netlify Functions では `included_files = ["knowledge/**"]` で同梱します
- 編集画面・保存確認フローは次フェーズです
